...
def Most_Relative_Sentence():
	...
	num=40 ##number of most relative sentences considering
...
def get_NER(qc,query):
	...
	for ii in range(len(Top_similarity_sent)):
		...
		for entity in doc.ents:
			nerr.append(entity.label_) ####### NEED TO MODIFY HERE FOR CHUNKING Pbm
	...
	if(len(random_ans)==0):
		...
		for ra_i in range(limit): ##### Taking 10 random answers from top K-ranked sentences by ignoring the stopwords.
			if not(words[ra_i] in query_tokens):
...
def writing(qa):
	...
	print("\n\n")
	print("len label_ans : ",len(label_ans))
	print("\n")
...
def Question_Classification(user_input):
	...
	print("test_Labels - train_Labels = ",len(test_Labels),len(train_Labels))
	...
	#nn = LogisticRegression(random_state=0, solver='sag', multi_class='multinomial')
...
out = open("output_files/Accuracy.txt","w") ## category wise answers [ write operation ]
out_m = open("output_files/Accuracy_of_All_Queries.txt","a") ## this file contain all categories answers [append operation ]


#input_file = open("Category_wise_Questions/whole_questions.txt","r").readlines()### calling all categories combinely:
input_file = open("Category_wise_Questions/date.txt","r").readlines()### calling category wise queries; once done with all categories we can sum up the results
...
for n,qa in enumerate(Questions):
	...
	print("qa: ",qa)
	with open("Category_wise_Questions/date.txt", "r") as f:
		lines = f.readlines()
	with open("Category_wise_Questions/date.txt", "w") as f:
		for line in lines:
		    if line.strip("\n") != qa.strip("\n"):
		        f.write(line)
	f.close()
